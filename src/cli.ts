    ...
    switch (cmd.action) {
      ...
      case "help": {
        return {
          ok: true,
          commands: [
            "launch - Start the driver (options: width, height, debug)",
            "close - Stop the app",
...
async function main() {
  ...
  if (args.includes("--help") || args.includes("-h")) {
    console.log(`
opentui-test Driver CLI

Headless driver protocol for OpenTUI apps.
This generic CLI launches the renderer without an app element.
To drive your own app over JSON lines, create your own CLI entry point that imports your app and passes it to createDriver({ app }).

Usage:
  bun src/cli.ts              # Interactive JSON-line mode
  bun src/cli.ts --help       # Show this help
...
Commands:
  {"action":"launch","width":100,"height":30,"debug":false}  Launch driver
  {"action":"capture"}                                           Get current frame
  {"action":"capture","save":"./frame.txt"}                      Capture and save to file
  {"action":"snapshot","name":"frame"}                           Save snapshot to ./snapshots/
...
Example:
  echo '{"action":"launch"}' | bun src/cli.ts
  echo '{"action":"capture"}' | bun src/cli.ts
`);
    process.exit(0);
  }
